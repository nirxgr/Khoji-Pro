...
export const register = async (req,res) => {
    ...
    try {
        ...
        let token;
        if (otpEntry) {
            token = jwt.sign({id: otpEntry._id}, process.env.JWT_SECRET, {expiresIn: "1h"});
            ...
            token = jwt.sign({id: newotp._id}, process.env.JWT_SECRET, {expiresIn: "1h"});
            await newotp.save();        
        }
...
export const resetPassword = async (req,res) => {
    ...
    try {
        ...
        if (isSamePassword) {
            return res.json({ success: false, message: "Old password cannot be used again." });
        }
